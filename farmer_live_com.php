<?php
// farmer_live_com.php - FIXED VERSION (Dark mode flash + Top content fixed)
session_start();
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'farmer') {
    header("Location: login_signup.php"); exit;
}
...
<head>
    ...
    <title>Live Chat • Farmer | RiceGuard AI</title>
    ...
    <style>
       ...
       .chat-outer-wrapper {
    min-height: calc(100vh - 120px);  /* REDUCED: More space for top content */
    height: calc(100vh - 120px);
    max-width: 1280px;
    margin: 0 auto;
    padding-top: 8px;  /* EXTRA TOP SPACING */
}
...
<body>
            ...
            .then(users => {
                ...
                users.forEach(user => {
                    if (user.role === 'technician') {
                        ...
                        div.innerHTML = `
                            <div class="w-10 h-10 bg-slate-300 dark:bg-slate-600 rounded-2xl flex items-center justify-center text-2xl">🔧</div>
                            <div>
                                <p class="font-medium text-slate-900 dark:text-white">${user.full_name}</p>
                                <p class="text-xs text-slate-500 dark:text-slate-400">Technician</p>
                            </div>
                        `;
    ...
    function openChat(chatId, name = '') {
        ...
        if (chatId === 'group') {
            ...
            document.getElementById('chatAvatar').innerHTML = '🔧';
            document.getElementById('chatAvatar').className = 'w-10 h-10 bg-slate-300 dark:bg-slate-600 rounded-2xl flex items-center justify-center text-3xl';
