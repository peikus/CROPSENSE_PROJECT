<?php
// admin_farmerlog.php - Farmers Management (Fixed Dropdown)
ob_start();
session_start();
...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("UPDATE users SET status=? WHERE id=? AND role='farmer'");
    $stmt->bind_param("si", $st, $id);
    $stmt->execute();
    $stmt->close();
    header("Location: admin_farmerlog.php?msg=" . urlencode($st === 'approved' ? 'Farmer approved successfully!' : 'Farmer request declined.'));
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: admin_farmerlog.php?msg=Farmer deleted successfully.");
    exit;
}
...
if (isset($_POST['update_user'])) {
    ...
    $stmt = $conn->prepare("UPDATE users SET full_name=?, email=?, status=? WHERE id=? AND role='farmer'");
    $stmt->bind_param("sssi", $name, $email, $st, $id);
    $stmt->execute();
    $stmt->close();
    header("Location: admin_farmerlog.php?msg=Farmer updated successfully.");
    exit;
}
...
// Load only Farmers
$stmt = $conn->prepare("SELECT * FROM users WHERE role = 'farmer' ORDER BY full_name");
$stmt->execute();
...
<head>
    ...
    <title>RiceGuard AI • Manage Farmers</title>
    ...
    <style>
        ...
        .dropdown-item-custom.success:hover {
        ...
        /* Hide Bootstrap dropdowns to avoid conflicts */
        .dropdown-menu {
        ...
        <div class="nxl-content">
            <div class="page-header">
                <div class="page-header-title">
                    <h5 class="m-b-10">Manage Farmers</h5>
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    <?php if (empty($farmers)): ?>
                                        <tr><td colspan="5" class="text-center py-5 text-muted">No farmers found.</td></tr>
                                    <?php else: foreach ($farmers as $row): ?>
                                    <tr>
